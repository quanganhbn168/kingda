            ...
            <aside class="lg:sticky lg:top-28 lg:self-start">
                <div class="overflow-hidden rounded bg-white shadow-xl ring-1 ring-slate-200">
                    <div class="bg-[#17110a] px-5 py-5 text-white">
                        <p class="text-xs font-extrabold uppercase text-red-200">{{ __('ui.common.kingda') }}</p>
                    ...
                    <a href="{{ url(app()->getLocale() === 'vi' ? '/san-pham' : '/' . app()->getLocale() . '/products') }}"
                        ...
                        <span class="rounded-full px-2 py-1 text-xs {{ $activeCategory ? 'bg-slate-100 text-slate-500 group-hover:bg-white group-hover:text-primary' : 'bg-white/20 text-white' }}">{{ $totalProductsCount ?? $products->total() }}</span>
                    </a>
                    ...
                    <div class="mt-3 space-y-2">
                        @foreach($categories as $category)
                            ...
                            <a href="{{ $category->translation?->public_url }}"
                                ...
                                <span class="rounded-full px-2 py-1 text-xs font-extrabold {{ $isActive ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-white group-hover:text-primary' }}">{{ $category->products_count }}</span>
                            </a>
                        @endforeach
                    </div>
                    </div>
                ...
                <div class="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                    <div>
                        ...
                        @if($activeCategory?->translation?->description)
                            <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{{ $activeCategory->translation->description }}</p>
                        @endif
                    </div>
                    <div class="text-sm font-bold text-slate-500">{{ __('ui.common.product_count', ['count' => $products->total()]) }}</div>
                </div>
                ...
                @if($products->count())
                    <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                        @foreach($products as $product)
                            ...
                            <a href="{{ $product->translation?->public_url }}" class="group block overflow-hidden rounded bg-white shadow ring-1 ring-slate-200 transition hover:-translate-y-1 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                                @if($productImage)
                                    <img src="{{ $productImage }}" alt="{{ $product->translation?->name }}" class="h-52 w-full object-cover transition duration-500 group-hover:scale-105">
                                @else
                                    <div class="flex h-52 items-center justify-center bg-gradient-to-br from-white via-red-50 to-slate-200 text-5xl text-primary">
                                        <i class="fa-solid fa-box-open"></i>
                                    </div>
                                @endif
                                <div class="p-5">
                                    <div class="flex items-center justify-between gap-3">
                                        <span class="text-xs font-extrabold uppercase text-primary">{{ $product->category?->translation?->name }}</span>
                                        @if($product->sku)
                                            <span class="text-xs font-bold text-slate-400">{{ $product->sku }}</span>
                                        @endif
                                    </div>
                                    <h3 class="mt-3 min-h-14 text-lg font-extrabold text-slate-950 transition group-hover:text-primary">{{ $product->translation?->name }}</h3>
                                    <p class="mt-3 min-h-20 text-sm leading-6 text-slate-600">{{ \Illuminate\Support\Str::limit($product->translation?->description, 120) }}</p>
